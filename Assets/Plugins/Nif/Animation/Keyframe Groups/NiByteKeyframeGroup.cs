...
{
	[Serializable]
	class NiByteKeyframeGroup : NiKeyframeGroup<byte>
	{
		public NiByteKeyframeGroup(int length, KeyType interpolation, BinaryReader reader) : base(length, interpolation, reader) { }

		protected override int AnimCurveCount => 1;

		protected override byte GetValue() => reader.ReadByte();
		...
		protected override byte CalculateLinearTangent(NiKeyframe<byte> index, NiKeyframe<byte> toIndex)
		{
			...
			return (byte)((index.value - toIndex.value) / num);
		}
